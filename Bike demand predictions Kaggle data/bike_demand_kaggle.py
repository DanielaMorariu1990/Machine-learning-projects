...
my_trans=ColumnTransformer(
  ...
  ("bin and encode atemp", KBinsDiscretizer(n_bins=3,encode="onehot-dense", strategy="uniform"),["atemp"]),
   ("interaction term work and hour", PolynomialFeatures(interaction_only=True, include_bias=False,degree=2),["workingday","hour"]),
...
sn.heatmap(abs(X_corr.corr()[["count"]]),cmap="RdBu_r",center=0.0, annot=True)
...
sn.heatmap(abs(X_corr[features_sub].corr()[["count"]]),cmap="RdBu_r",center=0.0, annot=True)
...
###CV for lin reg
cross_val_lin=cross_val_score(linear_regression, X_trans, y_train_trans, cv=5)
...
cross_val_sub1=cross_val_score(linear_regression2,X_trans, y_train_trans, cv=5)
...
feat_importance=pd.DataFrame(random_forest_base.feature_importances_, index=X_trans.columns, columns=["var"])
...
grid_search = GridSearchCV(random_forest_base, param_grid, cv=ShuffleSplit(n_splits=10))
my_search = grid_search.fit(X_trans,y_train_trans)

ranked_res1=pd.DataFrame(my_search.cv_results_).sort_values('rank_test_score')
...
import xgboost as xgb
label=y_train_trans
dtrain = xgb.DMatrix(X_trans, label=label)
param = {'max_depth': 3, 'eta': 0.5}
...
xgb=XGBRegressor(n_estimators=50,max_depth=5,learning_rate=0.1,random_state=42)
...
ranked_res_xgb=pd.DataFrame(my_search_xgb.cv_results_).sort_values('rank_test_score')
...
pd.DataFrame(my_search_xgb.best_estimator_.feature_importances_, index=X_trans.columns, columns=["var"]).sort_values(by="var",ascending=True).plot.barh()
plt.title("Optimized XGB feature importance")

#output=output.append(
 #   { "model": "GB LR Grid 0.3; n_est100 ","R2 mean": ranked_res_xgb[ranked_res_xgb["rank_test_score"]==1]["mean_test_score"].value,
  #  "R2 std":ranked_res_xgb[ranked_res_xgb["rank_test_score"]==1]["std_test_score"].value,
...
y_lin_reg=pd.concat([pd.Series(np.exp(y_pred_full)),pd.Series(y_train.index)],axis=1)
y_lin_reg.set_index("datetime",inplace=True)


y_lasso=pd.concat([pd.Series(np.exp(pred_lasso)),pd.Series(y_train.index)],axis=1)
y_lasso.set_index("datetime",inplace=True)

y_randrf=pd.concat([pd.Series(np.exp(pred_rand)),pd.Series(y_train.index)],axis=1)
y_randrf.set_index("datetime",inplace=True)

rf_grid=pd.concat([pd.Series(np.exp(rf_grid)),pd.Series(y_train.index)],axis=1)
rf_grid.set_index("datetime",inplace=True)

y_xgb=pd.concat([pd.Series(np.exp(pred_xgb_grid)),pd.Series(y_train.index)],axis=1)
...
X_test["peek_hours"]=[1 if x in [7,8,9,17,18,19] else 0 for x in X_test["hour"]]
...
rf_grid_test=pd.concat([pd.Series(np.exp(pred_XGV)),pd.Series(y_test.index)],axis=1)
rf_grid_test.set_index("datetime",inplace=True)

y_xgb_test=pd.concat([pd.Series(np.exp(pred_rand)),pd.Series(y_test.index)],axis=1)
...
#####Actual test data
bikes_test=pd.read_csv("test.csv",header=0, index_col="datetime", parse_dates=True)
bikes_test["hour"]=create_col_hour(bikes_test.index)
bikes_test["month"]=create_col_month(bikes_test.index)
bikes_test["peek_hours"]=[1 if x in [7,8,9,17,18,19] else 0 for x in bikes_test["hour"]]
...
y_xgb_pred=pd.concat([pd.Series(np.exp(pred_XGV_test)),pd.Series(bikes_test.index)],axis=1)

y_xgb_pred.columns=["count","datetime"]
y_xgb_pred.to_csv(r'C:/Users/Daniela/Documents/Spiced/Predicting_bike_demand/prediction_bike_demand_D.csv', index = False, header=True)